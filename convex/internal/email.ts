...
export const sendOTPEmail = internalAction({
  ...
  handler: async (ctx: ActionCtx, args): Promise<EmailResult> => {
    try {
      ...
      const emailData = {
        sender: {
          name: "Control Home Verification",
          email: FROM_EMAIL,
        },
        to: [
          {
        ...
        subject: "Your Control Home Verification Code",
        replyTo: {
          email: SUPPORT_EMAIL,
        ...
        htmlContent: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #eee; border-radius: 10px;">
            ...
            <p>Thank you,<br>The ctrlhome Team</p>
          </div>
        `,
      };
...
export const sendEmail = internalAction({
  ...
  handler: async (ctx: ActionCtx, args): Promise<EmailResult> => {
    try {
      ...
      const emailData: any = {
        sender: {
          name: "Control Home Notifications",
          email: FROM_EMAIL,
        },
        to: [
          {
